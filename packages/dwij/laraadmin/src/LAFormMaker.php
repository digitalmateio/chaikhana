				...
				case 'File':
					...
					if(isset($upload->id)) {
						$out .= "<a class='btn btn-default btn_upload_file hide' file_type='file' selecter='".$field_name."'>Upload <i class='fa fa-cloud-upload'></i></a>
							<a class='uploaded_file' target='_blank' href='".$upload->url."'><i class='fa fa-file-o'></i><i title='Remove File' class='fa fa-times'></i></a>";
					} else {
...
//		dump($params); return;
            ...
            {
               $label = $label.' '.$locale;
               $field_name = $field_name.'_'.$locale;
            }
			...
			switch ($field_type->name) {
				...
				case 'File':
					if($value != 0) {
						...
						if(isset($upload->id)) {
							$value = '<a class="preview" target="_blank" href="'.$upload->url.'">
